...
TEST(CppParserTest, HasFeatureResultValue) {
  ...
  info_data->mutable_cxx()->add_supported_predefined_macros("__has_feature");
  info_data->mutable_cxx()->add_supported_predefined_macros("__has_extension");
  info_data->mutable_cxx()->add_supported_predefined_macros("__has_attribute");
  ...
  m->set_key("feature");
  m->set_value(2);
  m = info_data->mutable_cxx()->add_has_extension();
  m->set_key("extension");
  m->set_value(3);
  m = info_data->mutable_cxx()->add_has_attribute();
  m->set_key("attribute");
  m->set_value(4);
  m = info_data->mutable_cxx()->add_has_cpp_attribute();
  m->set_key("cpp_attribute");
  m->set_value(5);
  m = info_data->mutable_cxx()->add_has_declspec_attribute();
