#!/usr/bin/env python
# Copied from chromium build/ and modified for Goma.
#
...
def WindowsSDKDirInCipdCache():
  """Returns Windows SDK dir if it has Windows SDK in CIPD cache."""
  # TODO: better way to detect Windows SDK installed by CIPD.
  vsinstalldir = os.environ.get('VSINSTALLDIR', '')
  if vsinstalldir and os.path.isdir(vsinstalldir):
    return vsinstalldir
  return None
...
def SetEnvironmentAndGetRuntimeDllDirs():
  ...
  cipd_win_toolchain = \
      bool(int(os.environ.get('CIPD_WIN_TOOLCHAIN', '1')))
  depot_tools_win_toolchain = \
      bool(int(os.environ.get('DEPOT_TOOLS_WIN_TOOLCHAIN', '1')))
  # TODO: better way to detect Windows SDK installed by CIPD.
  cipd_cache_windows_sdk_dir = WindowsSDKDirInCipdCache()
  if (sys.platform in ('win32', 'cygwin') and cipd_cache_windows_sdk_dir
      and cipd_win_toolchain):
    vs_runtime_dll_dirs = [
        os.path.join(cipd_cache_windows_sdk_dir, 'sys64'),
        os.path.join(cipd_cache_windows_sdk_dir, 'sys32'),
    ]
    os.environ['GYP_MSVS_OVERRIDE_PATH'] = cipd_cache_windows_sdk_dir
    with open(os.path.join(cipd_cache_windows_sdk_dir, 'VS_VERSION')) as f:
      os.environ['GYP_MSVS_VERSION'] = f.read().strip()
    os.environ['WINDOWSSDKDIR'] = os.path.join(cipd_cache_windows_sdk_dir,
                                               'win_sdk')
  # When running on a non-Windows host, only do this if the SDK has explicitly
  # been downloaded before (in which case json_data_file will exist).
  elif ((sys.platform in ('win32', 'cygwin') or os.path.exists(json_data_file))
  ...
  for debug_file, is_optional in debug_files:
    ...
    if not os.path.exists(full_path):
      if is_optional:
        ...
        # TODO: remove version requirement.
...
def Update(force=False):
  ...
  if (sys.platform in ('win32', 'cygwin')
      and cipd_win_toolchain and WindowsSDKDirInCipdCache()):
    # Installed toolchain will be used. We do not need to do anything.
    return 0
